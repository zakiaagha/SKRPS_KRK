...
class User{
	...
	function insertRole(){
		$query = "insert into krk_role_detail (rd_user_id, rd_role_id, rd_cr_uid, rd_cr_dt) VALUES(?,?,?,?)";
		$stmt = $this->conn->prepare($query);
	...
	function readOne(){		
		$query = "SELECT * FROM ".$this->table_name." LEFT JOIN ".$this->table_role." ON rd_user_id=idm_user LEFT JOIN krk_roles ON idm_role=rd_role_id WHERE idm_user=? LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
	...
	function deleteRole(){
		$query = "DELETE FROM " . $this->table_role . " WHERE rd_user_id=?";
